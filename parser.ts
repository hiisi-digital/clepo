import { Arg, ArgAction, CommandInstance, CommandMetadata } from "./types.ts";
...
export class Parser {
  ...
  private parseTokens(
    ...
    for (let cursor = 0; cursor < args.length; cursor++) {
      ...
      if (token.startsWith("-")) {
        ...
        this.applyValue(instance, argDef, action, value);
  ...
  private validateRequired(instance: CommandInstance, meta: CommandMetadata) {
    for (const arg of meta.args.values()) {
      if (arg.required && (instance as any)[arg.id!] === undefined) {
   ...
   */
  private applyValue(target: any, arg: Arg, action: ArgAction, value: any) {
    const key = arg.id!;
    switch (action) {
      case ArgAction.Set:
        target[key] = value;
